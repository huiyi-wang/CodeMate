...
**混合项目**：设 `primary` + `secondary`（如 wjh_ocr = engineering + inference），两镜头各出一节，不强行单标签。
...
**为何本地有 git 仍要 clone**：`git diff` 看不见未跟踪新增（如 `*_guopei.py`）、本地删除的上游文件、与 `origin` 同 commit 但工作区大改等；**树级 diff 才是完整基线**。
